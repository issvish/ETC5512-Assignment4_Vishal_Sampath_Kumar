...
This map places the parking sensor observations on the municipal boundary of the City of Melbourne, which makes the spatial pattern easier to interpret than a plot of points alone. It shows where occupied and unoccupied observations are concentrated within the municipality rather than simply displaying them against blank axes.

The spatial plot shows that the sensor points are clustered within a fairly concentrated part of the Melbourne municipal boundary, rather than being evenly spread across the entire map. The red points, which indicate **occupied observations, clearly outnumber the grey unoccupied points** in the central cluster, visually matching the earlier summaries showing occupancy rates above 0.5 for many periods.

This map does not by itself prove which zones are the most persistently congested, but it does provide useful context. It shows that the observed parking activity is spatially concentrated and that occupied observations are common across the central cluster of parking sensor locations.
...
Looking across all findings together, the analysis suggests that parking congestion in this sample is both spatially concentrated and strongly time-dependent. Several zones have an occupancy rate of 1.00, and several others also have a persistence score of 1.00, meaning that **repeated congestion is not confined to a single isolated case**.

The time-of-day pattern is especially striking. Occupancy dips in the middle of the day and then rises into the evening, peaking around 18:00. That means **the most intense parking pressure in this sample does not occur at midday, but later in the day**.
...
Overall, the tables, charts, and maps all point in the same direction. **Parking congestion in this sample appears strongest in a subset of zones, higher on weekdays than weekends, and especially pronounced in the late afternoon and evening**.
...
Several zones appear persistently occupied, and the hourly summaries suggest that the strongest parking pressure is not constant through the day. The day-of-week section adds another layer by showing whether those patterns are tied more strongly to weekday or weekend behaviour. Together, the time-based and spatial summaries suggest that the most frustrating parking conditions are likely to be concentrated both geographically and temporally, rather than being uniform across the city.

The most important finding is that **congestion does not appear to be evenly spread across all zones or all times**. Instead, **it is concentrated in central zones, stronger on weekdays, and especially high in the evening**.
